...
description: Sincroniza y mantiene la integridad de los enlaces simbólicos (symlinks) de skills y agentes desde ai-specs/ hacia .claude/, .cursor/ y .agents/.
author: Focuz AI
version: 1.0.0
---
...
Esta skill permite mantener alineados los directorios de copilots específicos (`.claude/`, `.cursor/`, `.agents/`) con la fuente canónica de agentes y skills del proyecto: `ai-specs/`.
...
- **Solo Symlinks**: Solo se deben gestionar entradas que correspondan a enlaces simbólicos que apunten a `ai-specs/`. Nunca elimine directorios reales o archivos locales creados por el usuario en las carpetas espejo.
...
Ejecutar las operaciones utilizando comandos de terminal según el sistema operativo (en Windows, mediante PowerShell `New-Item -ItemType SymbolLink` o `cmd /c mklink`):
