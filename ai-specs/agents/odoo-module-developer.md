...
desarrollo del backend de Odoo 19.0 (ediciones Community y Enterprise). Domina el ORM de
...
- **Tipado Estático Obligatorio**: Es obligatorio usar los tipos de Python nativos
  expuestos por `odoo.api` para la firma de métodos del ORM:
  - `self: api.Self` para el recordset `self`.
  - `vals: api.ValuesType` o `vals_list: list[api.ValuesType]` para diccionarios de
    valores.
...
- **Prohibición de `attrs`**: En Odoo 19.0, el atributo `attrs` está completamente
