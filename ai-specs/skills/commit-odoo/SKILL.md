...
un Pull Request en GitHub, adhiriéndose al estándar oficial de contribuciones de Odoo 16.0.
...
- **Línea de Asunto**: Debe seguir la estructura: `[TAG] nombre_modulo: descripción corta del cambio (< 50 caracteres)`
  - _Ejemplo_: `[FIX] account_invoice_local: corregir redondeo de impuestos`
  - _Tags permitidos_: `[ADD]`, `[FIX]`, `[IMP]`, `[REF]`, `[REM]`, `[CLN]`, `[PERF]`, etc.
...
- **Descripción Larga (Cuerpo)**:
  ...
  - Explique el **POR QUÉ (WHY)** se hizo el cambio, no el _qué_ (el qué ya está visible en el código).
