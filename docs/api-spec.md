---
description:
  Especificación técnica de la API de Odoo, incluyendo endpoints de JSON-RPC,
  XML-RPC y la especificación de controladores REST personalizados en español.
alwaysApply: true
---

# Especificación de la API de Odoo (API Spec)

Este documento detalla los estándares de integración y comunicación externa con Odoo
Odoo expone servicios nativos mediante **XML-RPC** y **JSON-RPC**, además de
...
El núcleo de Odoo se expone por defecto mediante servicios RPC en el puerto configurado
(ej. `8069`).
...
Para integraciones móviles o con plataformas de terceros donde se requiere una API REST
JSON limpia, se configuran controladores en Odoo heredando de `http.Controller`.
